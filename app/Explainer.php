...
class Explainer
{
     ...
     * @return
     */
    public function performQueryAnalysis()
    {
        if (preg_match('/^\\s*SELECT\\s*`?[a-zA-Z0-9]*`?\\.?\\*/i', $this->query)) {
            $this->hints[] = 'Use <code>SELECT *</code> only if you need all columns from table';
        }
        if (preg_match('/ORDER BY RAND()/i', $this->query)) {
            $this->hints[] = '<code>ORDER BY RAND()</code> is slow, try to avoid if you can. You can <a href="http://stackoverflow.com/questions/2663710/how-does-mysqls-order-by-rand-work">read this</a> or <a href="http://stackoverflow.com/questions/1244555/how-can-i-optimize-mysqls-order-by-rand-function">this</a>';
        }
        ...
        if (preg_match('/LIMIT\\s/i', $this->query) && stripos($this->query, 'ORDER BY') === false) {
